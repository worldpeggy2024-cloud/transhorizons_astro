/*
 * TransHorizons — France Country Analysis Data (LEGACY, single-phase).
 *
 * NOTE (2026-07-28): the shared TYPE contract moved to ./countries/analysisTypes.
 * This file now holds only France's legacy hardcoded data — which is itself DEAD:
 * the renderer reads France from france-yaml.ts, nothing imports franceAnalysis.
 * The re-export below is a TRANSITIONAL shim so disposable country data files that
 * still 'import ... from ./france' keep resolving until they are regenerated.
 * TODO(post-migration): delete this file once France is regenerated off analysis.yaml
 * and no data file imports from it (permanent code already imports from analysisTypes).
 */
import type { AnalysisContent } from './countries/analysisTypes';
// Transitional re-export shim — see header. Remove with this file at France regen.
export type {
  ScoreRating,
  AnalysisContent,
  LangContent,

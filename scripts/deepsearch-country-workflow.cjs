...
PER ITEM: gap (one sentence, close paraphrase — the report speaking, not the composer, with inline markers); anchor ([dot.path] field(s) and [source-id](s), at least one, all resolving; the bare situation is a valid field anchor — a gap asserted only in a situation thread anchors situation plus the event's source id); since (when first officially identified, or the span the report gives, cited; 'report-silent' where the sources carry no duration — never fabricate); class (Interpretation — assign by DECISION TABLE, first row that matches, each row keyed to what the report STATES, never to an inference from a rating movement alone: (1) report states remediation under way and does not state failure/regression → in-progress-unclosed; (2) report states an announcement/commitment/plan and no implementation → announced-not-implemented; (3) report states an attempt or a NAMED standing remediation record AND states the gap persisted or the area regressed → attempted-and-failed; (4) report states no attempt of any kind and carries a source that would have recorded one → never-attempted. Tie-breakers: a worsening rating with no named attempt behind it is never-attempted — a measurement is not an attempt; a named remediation record with stated regression IS an attempt record; ongoing effort + no closure → row 1 unless the report states failure).
...
economy.politicalEconomy: OPENER (required, one sentence): the state–market configuration before any distributional detail — state-directed / mixed / market-led; the role of Crown corporations, state-owned enterprises or sovereign funds where they exist; where the boundary is currently contested. Then: who benefits and who loses under the current model; business elite structure — NAMING the load-bearing entities (largest firms, principal producers of the leading sectors, main labour federations), each cited; what reforms are technically necessary vs. politically possible. NAMED-ACTOR RULE applies: a nameless "business elite" or producer-less sector is an under-sourced claim.

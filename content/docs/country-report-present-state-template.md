...
> `political · economy · society · security · actors · risks · sources`.
>
...
1. **Society is now a top-level peer** of `political`, `economy`, and `security` — same rank, same sourcing
   rigor, its own collapsible card, slotted **after `economy` and before `security`**. Previously, society
   existed only as scattered inputs inside `stabilityDrivers` and `security.internal` — i.e. society was a
   *risk factor*, never a *component of the country in itself*.
...
## 1. The four peers, and the society principle

A country is described as four peers, each on its own terms:

`political` · `economy` · **`society`** · `security`

**Society is described *before* and *independent of* any stability implication.** A society is not an input
to "does the regime hold" — it is a thing that exists whether or not it threatens anything.
...
1. **Pass A — sources only.** Harvest candidate sources covering macro, governance, trade, security, **and
   society** (demography, composition, religion, cohesion). Sourcing tool of your choice.
2. Paste into `[code].sources.yaml`; run the validator on the sources block. If it fails, **do not write
   prose.**
3. **Pass B — prose from approved source IDs only.** Now covering the society section.
...
6. Open the rendered page and review visually — **including the new society card** (citations resolve, FR
   toggle works).
...
Required: one candidate source list covering **macro, governance, trade, security, and society** (the four
society sub-domains: demography, composition, religion, cohesion); full deep links only, never homepages; all
8 schema fields per source; provisional `id` values; `citationType` set for every source.
...
## 13. Output structure (17 blocks — society inserted after economy)

1. `executiveSnapshot` — bullets with inline `[source-id]`, **including a society line** (see §14)
2. `political.powerStructure`
3. `political.stabilityDrivers` — *(elite cohesion lives here)*
4. `political.shockAbsorbers`
5. `economy.macroReality`
6. `economy.externalVulnerability`
7. `economy.politicalEconomy`
8. **`society.demographics`** — population structure; median age / youth-bulge or ageing reality; urban/rural split; migration patterns. Time-bound.
9. **`society.composition`** — ethnic, linguistic, religious composition; where fault lines run; **cross-cutting vs reinforcing** geometry (§9).
10. **`society.religion`** — composition + fault line; lived/syncretic texture; political salience; bias documented (§8).
11. **`society.cohesion`** — interpersonal & institutional trust, social capital, lived self-image. Barometer/WVS/Pew as **primary** source.
12. `security.internal`
13. `security.diplomacy`
14. `actors.domestic`
15. `actors.external`
16. `risks`
17. `sources` — appears once, at the end; every cited source, all 8 fields; no orphans either way.
...
1. EXECUTIVE SNAPSHOT (11 bullets)
...
4. SOCIETY (describe the society ON ITS OWN TERMS, before and independent of any stability implication; a
...
6. KEY ACTORS (domestic and external)
...
economy.macroReality, economy.externalVulnerability, economy.politicalEconomy, society.demographics,
society.composition, society.religion, society.cohesion, security.internal, security.diplomacy,
actors.domestic, actors.external, risks. End with a complete ## sources block in YAML, all 8 fields per

...
| `since` | When it was first officially identified, or a duration the report states, cited. **A publication, edition, or measurement date is not a `since`** — "the 2023 report counted it" tells the reader when someone last measured, not how long the gap has stood, and rendering it as a duration is a false claim. If a vintage is all the report carries, use `report-silent` and record the item in `undatedGaps`. **Never invent a date.** **Narrow exception — `since` only:** a `since` value MAY cite a `[source-id]` that appears nowhere else in the report's prose. Identification dates are sourced by the targeted since-dates harvest that runs *after* the register is composed, so its establishing source is often referenced only from the `since`. The exception is scoped strictly to the `since` attribute — the gap *claim* itself must still be asserted, with a resolvable `[source-id]`, in a peer field or situation thread; and the cited source must state the date (the publication/edition/measurement bar above still applies). |
